...
public sealed class Plugin : BasePlugin
{
    ...
    public override void Load()
    {
        ...
        PositionY = Config.Bind("Overlay", "Y", 250f, "Overlay Y position in pixels.");
        ...
        // Migrate the original cramped layout while preserving custom user positions.
        if (Math.Abs(PositionX.Value - 24f) < 0.1f) PositionX.Value = 28f;
        if (Math.Abs(PositionY.Value - 120f) < 0.1f) PositionY.Value = 250f;
